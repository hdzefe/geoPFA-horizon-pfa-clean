...
class ThermalDataLoader:
    ...
    def load_borehole_data_all_horizons(self):
        """Load borehole data from ALL 6 horizons and find complete coverage"""
        horizons = {
    ...
    def interpolate_temperature_grid_hybrid(self, mean_depth_surface, borehole_gdf):
        """Interpolate temperature using HYBRID approach with complete 6-horizon filtering"""
        logger.info(f"    Interpolating temperature (HYBRID: GeoTIS spatially-filtered + gradient fallback)...")
        
        # Load all horizons and find boreholes with COMPLETE coverage
        ...
        logger.info(f"      ✓ Filtered to {len(borehole_gdf_filtered)}/{len(borehole_gdf)} boreholes with complete 6-horizon coverage")
        
        if len(borehole_gdf_filtered) < 3:
            logger.warning(f"      ⚠ Less than 3 boreholes with complete coverage")
            logger.info(f"      Using all boreholes")
        ...
        if self.geoTIS_data:
            logger.info(f"      Querying GeoTIS at {len(borehole_gdf_filtered)} borehole locations...")
            ...
            logger.info(f"      GeoTIS temperatures found: {geotis_count}/{len(borehole_gdf_filtered)} boreholes")
        ...
        logger.info(f"      RBF interpolating blended temperatures...")
...
def main():
    ...
    logger.info("\n" + "="*80)
    logger.info("THERMAL EVIDENCE LAYERS (Borehole-based HYBRID with Complete Coverage)")
    logger.info("="*80)
    ...
    for horizon_name, filename in horizons.items():
        logger.info("\n" + "="*80)
        logger.info(f"Processing: {horizon_name}")
        logger.info("="*80)
        ...
        try:
            ...
            logger.error(f"✗ Error processing {horizon_name}: {e}", exc_info=True)
            continue
