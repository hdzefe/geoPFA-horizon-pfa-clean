...
class ThermalDataLoader:
    ...
    def load_geoTIS_temperature_data(self):
        ...
        for data_file in data_files:
            try:
                ...
                logger.info(f"  ✓ {data_file.name}: {len(df)} points, T range [{df['Temperature'].min():.1f}, {df['Temperature'].max():.1f}]°C")
    ...
    def process_tier1_horizon(self, horizon_name):
        ...
        try:
            ...
            # Get top and base temperatures
            top_temp = griddata(
                np.column_stack([self.geoTIS_data[min(self.geoTIS_data.keys())]['X'],
                                self.geoTIS_data[min(self.geoTIS_data.keys())]['Y']]),
                self.geoTIS_data[min(self.geoTIS_data.keys())]['T'],
                (np.linspace(self.extent['left'], self.extent['right'], self.grid_size[0]),
                 np.linspace(self.extent['bottom'], self.extent['top'], self.grid_size[1])),
                method='nearest'
            )
